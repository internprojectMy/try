<?php
	...
	$col_set = array('T0.cash_name','T0.cash_day');
	...
	if (empty($order_by)){
	    $order_by = " ORDER BY T0.cash_id DESC ";
	}
	...
	while ($row = mysqli_fetch_assoc($sql)){
		...
		$data['DT_RowId'] = "row_".$row['cash_id'];
  ...
  $filtered_row_count_query = "SELECT
								COUNT(T0.cash_id) AS ROW_COUNT,
								T0.cash_name,
								T0.cash_day,
								T0.status,
							FROM
							cash_collector".$where;
	...
	$full_row_count_sql = mysqli_query($con_main, "SELECT COUNT(T0.cash_id) AS C FROM cash_collector AS T0");
