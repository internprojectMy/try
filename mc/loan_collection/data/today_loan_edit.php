...
$collectid = (isset($_REQUEST['id']) && $_REQUEST['id'] != NULL && !empty($_REQUEST['id'])) ? $_REQUEST['id'] : 0;
...
$loan_detail_query = "SELECT
						...
						mas_user.EMP_NO,
						mas_user.FIRST_NAME,
						loan_customer.id AS CUS_NAME,
						loan_collecting.late_amount,
						loan_collecting.bill_amount,
						loan_collecting.total_amount,
						loan_collecting.collect_date
					FROM
						loan_lending AS LL
					INNER JOIN loan_type ON LL.loan_type = loan_type.id
					INNER JOIN mas_user ON LL.collector_id = mas_user.USER_CODE
					INNER JOIN loan_collecting ON loan_collecting.loan_id = LL.id
					INNER JOIN loan_customer ON LL.customer_id = loan_customer.id
					WHERE
						loan_collecting.id = '$collectid'";
...
$customer = $loan_detail_res['CUS_NAME'];
$loan = $loan_detail_res['LOAN_ID'];
$billamt = $loan_detail_res['bill_amount'];
$lateamt = $loan_detail_res['late_amount'];
$totalamt = $loan_detail_res['total_amount'];
$collect = $loan_detail_res['collect_date'];
...
$data[0]['collectid'] = $collectid;
$data[0]['customer'] = $customer;
$data[0]['loanid'] = $loan;
$data[0]['collect'] = $collect;

$data[0]['bill'] = $billamt;
$data[0]['late'] = $lateamt;
$data[0]['total'] = $totalamt;
