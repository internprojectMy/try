...
require_once ('../../reporting_part/report_header_oa10.php');
...
$query4 = "SELECT
				...
				WHERE
				loan_transactionexp.expcat = 'Administration Expenses' 
				GROUP BY
				loan_transactionexp.expcat,
				loan_transactionexp.expencestype,
				loan_expenses.category";			
...
$query5 = "SELECT
				...
				INNER JOIN branch ON loan_transactionexp.branch_name = branch.branch_id
				WHERE
				loan_transactionexp.expcat = 'Marketing Expenses' 
				GROUP BY
				loan_transactionexp.expcat,
				loan_transactionexp.expencestype,
				loan_expenses.category";
...
$query7 = "SELECT
				...
				INNER JOIN branch ON loan_transactionexp.branch_name = branch.branch_id
				WHERE
				loan_transactionexp.expcat = 'Finance & Other Expenses' 
				GROUP BY
				loan_transactionexp.expcat,
				loan_transactionexp.expencestype,
				loan_expenses.category";				
	...
	<!--====================================== DOCUMENT CHARGES TABLE =================================-->
		...
		<?php
			...
			while ($row = mysqli_fetch_array ($sql7)){
				...
				$tot_amt = $tot_amt + $row[''];
				$tot_interest = $tot_interest + $row['Sum(document_charges.document_amount)'];
...
<!-- 	<tr>
 ...
 -->
		...
		<td colspan="2"><strong><center> TOTAL</center></strong></td>			
			...
			<td><strong><?php echo (number_format($balance + $tot_interest,2)); ?></strong></td>
...
<!-- 	<tr>
	...
	<tr>
		...
		<td><strong><?php echo (number_format($othertotle - $totalexpense ,2)); ?></strong></td>	
