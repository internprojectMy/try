<?php
	...
	T0.DES_CODE
	FROM
	mas_designation AS T0".$where." ".$order_by."
	LIMIT ".$start.", ".$length;
	...
	while ($row = mysqli_fetch_assoc($sql)){
		...
		$data['DT_RowId'] = "row_".$row['DES_CODE'];
	...
	IFNULL(COUNT(T0.DES_CODE), 0) AS ROW_COUNT
	FROM
	mas_designation AS T0".$where;
	...
	$full_row_count_sql = mysqli_query($con_main, "SELECT COUNT(T0.DES_CODE) AS C FROM mas_designation AS T0");
