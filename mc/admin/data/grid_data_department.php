<?php
	...
	$col_set = array('T0.DEPARTMENT','T1.LOCATION');
	...
	T1.LOCATION
	FROM
	mas_department AS T0
	LEFT JOIN mas_location AS T1 ON T1.LOC_CODE = T0.LOC_CODE".$where." ".$order_by."
	LIMIT ".$start.", ".$length;
	...
	while ($row = mysqli_fetch_assoc($sql)){
		...
		$data['DT_RowId'] = "row_".$row['DEP_CODE'];
	...
	IFNULL(COUNT(T0.DEP_CODE), 0) AS ROW_COUNT
	FROM
	mas_department AS T0
	LEFT JOIN mas_location AS T1 ON T1.LOC_CODE = T0.LOC_CODE".$where;
	...
	$full_row_count_sql = mysqli_query($con_main, "SELECT COUNT(T0.DEP_CODE) AS C FROM mas_department AS T0");
